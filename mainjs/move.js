...
$(document).ready(function() {
  ...
  var onoff=true; // true=>타이머 동작중 , false=>동작하지 않을때
     ...
     $('.btn1').css('background','#e74d27'); //첫번째 불켜
    $('.btn1').css('width','30');
 ...
 function moveg(){
      ...
      $('.btn'+cnt).css('background','#e74d27');//자신만 불켜
      $('.btn'+cnt).css('width','30');   
                           
		$('.vgallery').animate({left:position}, 'slow'); //겔러리 무비의 left값을 움직여라~
  ...
  $('.mbutton').click(function(event){  //각각의 버튼을 클릭한다면...
	...
	if($target.is('.btn1')){  //첫번째 버튼을 클릭했다면...
    	   $('.vgallery').animate({left:0}, 'slow');
                cnt=1;
                direct=1;
	}else if($target.is('.btn2')){ //두번째 버튼을 클릭했다면...
    	   $('.vgallery').animate({left:-2000}, 'slow');
                cnt=2;
	}else if($target.is('.btn3')){ //세번째 버튼을 클릭했다면...
    	   $('.vgallery').animate({left:-4000}, 'slow');
                cnt=3;
	}else if($target.is('.btn4')){  //네번째 버튼을 클릭했다면...
    	   $('.vgallery').animate({left:-6000}, 'slow');
                cnt=4;
                direct=-1
	}
      
    $('.btn'+cnt).css('background','#e74d27');//자신 버튼만 불켜 
    $('.btn'+cnt).css('width','30');
